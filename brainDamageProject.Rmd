...
hospital_data$`Дата перевода в ФНКРР`[191] <- "09-11-2018"

hospital_data <- hospital %>% mutate(`Дата мозговой катастрофы` = format(as.Date(as.Date("1899-12-30") + as.numeric(`Дата мозговой катастрофы`), "%d-%m-%Y"), "%d-%m-%Y"),
                     `Дата перевода в ФНКРР` = format(as.Date(as.Date("1899-12-30") + as.numeric(`Дата перевода в ФНКРР`), "%d-%m-%Y"), "%d-%m-%Y"))

hospital_data$`Пол`
hospital_data %>% filter (str_detect(`Пол`, "м"))
...
СОСТОЯНИЕ ПАЦИЕНТОВ В ТОЧКЕ 0

```{r}
#ПОЛ
ggplot(hospital_days_var) +
  geom_bar (aes(`Пол`))

#ВОЗРАСТ
ggplot(hospital_days_var) +
  geom_histogram (aes(`Возраст`)) # ОШИБКА
