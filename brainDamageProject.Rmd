...
data <- hospital %>% mutate(`Дата мозговой катастрофы` = dmy(`Дата мозговой катастрофы`),
...
data <- data %>% mutate (`До_перевода_ФКЦРР_д` = as.Date(hospital_renamed $`Дата_перевода_ФНКЦРР` , format="%d-%m-%Y")- as.Date(hospital_renamed $`Дата_мозговой_катастрофы`, format="%d-%m-%Y")) %>% mutate(`Потребность_ИВЛ` = ifelse(`Потребность_ИВЛ` =="н/д", NA, `Потребность_ИВЛ`), `Исход` = ifelse( `Исход` == 2, NA, `Исход`),
