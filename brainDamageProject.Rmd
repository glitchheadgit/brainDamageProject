...
```{r }
data$`Дата_перевода_ФНКЦРР`[191] <- "09.11.2018"
data$`Дата_мозговой_катастрофы`[8] <- "01.01.2014"
data$`Дата_мозговой_катастрофы`[169] <- "12.09.2017"
data$`Дата_мозговой_катастрофы`[321] <- "01.01.2014"

data <- data %>% mutate(`Дата_мозговой_катастрофы` = dmy(`Дата_мозговой_катастрофы`),
                     `Дата_перевода_ФНКЦРР` = dmy(substr(`Дата_перевода_ФНКЦРР`, start=1, stop=8)),
                     `Дата_выписки_ФНКЦРР` = dmy(`Дата_выписки_ФНКЦРР`),
                     `До_перевода_ФКЦРР_д` = as.numeric(`Дата_перевода_ФНКЦРР` - `Дата_мозговой_катастрофы`)
                     )

data <- data %>% mutate(`Потребность_ИВЛ` = ifelse(`Потребность_ИВЛ` =="н/д", NA, `Потребность_ИВЛ`), `Исход` = ifelse( `Исход` == 2, NA, `Исход`),
...
data %>%
  ...
  labs(y="Number of patients", y = "Diagnosis group")
```
...
target_columns_renamed <- c(
  "Charlson_Comorbidity_Index_1d",
  "CGS_Score",
  "Age",
  "FOUR_1d",
  "SOFA_1d",
...
data <- data %>%
  rename(
    Charlson_Comorbidity_Index_1d = `Индекс_коморбидности_Чарлсона_1d`,
    CGS_Score = `CGS_балл`,
    Age = `Возраст`,
    FOUR_1d = `FOUR_1d`,
    SOFA_1d = `SOFA_1d`,
    ...
    `LYM_#_1d` = `LYM_#_1d`,
    PLT_1d = `PLT_1d`,
    Lactate_1d = `Лактат_1d`,
    Creatinine_1d = `Креатинин_1d`,
    ALT_1d = `АЛТ_1d`,
    AST_1d = `АСТ_1d`,
    Albumin_1d = `Альбумин_1d`,
    Alkaline_Phosphatase_1d = `Щелочная_фосфотаза_1d`,
    Urea_1d = `Мочевина_1d`,
    Uric_Acid_1d = `Мочевая_кислота_1d`,
    GGT_1d = `ГГТ_1d`,
    Potassium_1d = `Калий_1d`,
    Sodium_1d = `Натрий_1d`,
    APTT_1d = `АЧТВ_1d`,
    Chloride_1d = `Хлорид_1d`,
    INR_1d = `МНО_1d`,
    Fibrinogen_1d = `Фибриноген_1d`,
    PT_1d = `ПВ_1d`,
    oneday_log = `oneday_log`,
    `8ohdg_1d_log` = `8ohdg 1d log`,
    Ventilation_d = `ИВЛ_д`
  )

data_filtered <- data %>% select(all_of(target_columns_renamed))
...
ggplot(correlation_matrix_melted, aes(Var1, Var2, fill = value)) +
  ...
  labs(title = "Correlation matrix", x = "Variables", y = "Variables") -> corr_plot
...
Logistic regression for outcome and clinical scales

```{r}
data <- data %>% 
  rename(Outcome = `Исход` )
data$Outcome <- as.numeric(data$Outcome)
data %>%
  mutate(across(`Outcome`, function(x) x %>% factor(levels = c("0", "1"))))
data <- data %>%
  mutate(across(`Outcome`, as.factor))
data$SOFA_1d <- as.numeric(data$SOFA_1d)
data$Charlson_Comorbidity_Index_1d <-as.numeric(data$Charlson_Comorbidity_Index_1d)
data$CGS_Score<-as.numeric(data$CGS_Score)
data$oneday_log <-as.numeric(data$oneday_log)
data$`8ohdg_1d_log`<-as.numeric(data$`8ohdg_1d_log`)
data$Age <- as.numeric(data$Age)
model <- glm(Outcome ~ Age + SOFA_1d + Charlson_Comorbidity_Index_1d + 
               CGS_Score + oneday_log + `8ohdg_1d_log`, 
             data = data, 
             family = binomial)
summary(model)
coef(model)
model %>% tidy(exponentiate = TRUE, conf.int = TRUE)
model %>% glance()

rename_labels <- c(
  "Age" = "Age",
  "SOFA_1d" = "SOFA Score (1st Day)",
  "Charlson_Comorbidity_Index_1d" = "Charlson Comorbidity Index (1st Day)",
  "CGS_Score" = "CGS Score",
  "oneday_log" = "Extracellular DNA",
  "8ohdg_1d_log" = " DNA (Deoxyguanosine)"
)
...
p <- plot_model(model, 
                ...
                show.p = TRUE, 
                ci.lvl = 0.95, 
                title = "Clinical Scales, Extracellular DNA, and Outcome",
                axis.labels = rev(rename_labels))
p
data_clean <- data %>% drop_na(Age, SOFA_1d, Charlson_Comorbidity_Index_1d, 
                               CGS_Score, oneday_log, `8ohdg_1d_log`, Outcome)

model_full <- glm(Outcome ~ Age + SOFA_1d + Charlson_Comorbidity_Index_1d + 
                    CGS_Score + oneday_log + `8ohdg_1d_log`, 
                  data = data_clean, 
                  family = binomial)
model_reduced <- glm(Outcome ~ Age, 
                     data = data_clean, 
                     family = binomial)
...
```
Logistic regression, interactions and centering
...
model_dag <- glm(Outcome ~ Age + SOFA_1d + Charlson_Comorbidity_Index_1d + 
                   CGS_Score + oneday_log + `8ohdg_1d_log` +
                   Age:SOFA_1d + Age:Charlson_Comorbidity_Index_1d +
                   Age:CGS_Score + Charlson_Comorbidity_Index_1d:SOFA_1d + Charlson_Comorbidity_Index_1d:CGS_Score, 
                 data = data, 
                 family = binomial)
summary(model_dag)
coef(model_dag)
model_dag %>% tidy(exponentiate = TRUE, conf.int = TRUE)
vif_results_dag <- vif(model_dag)

data_center <- data

data_center$Age <- scale(data$Age, center = TRUE, scale = FALSE)
data_center$SOFA_1d <- scale(data$SOFA_1d, center = TRUE, scale = FALSE)
data_center$Charlson_Comorbidity_Index_1d <- scale(data$Charlson_Comorbidity_Index_1d, center = TRUE, scale = FALSE)
data_center$CGS_Score <- scale(data$CGS_Score, center = TRUE, scale = FALSE)
data_center$oneday_log <- scale(data$oneday_log, center = TRUE, scale = FALSE)
data_center$`8ohdg_1d_log` <- scale(data$`8ohdg_1d_log`, center = TRUE, scale = FALSE)



model_dagcentr <- glm(Outcome ~ Age + SOFA_1d + Charlson_Comorbidity_Index_1d + 
                        CGS_Score + oneday_log + `8ohdg_1d_log` +
                        Age:SOFA_1d + Age:Charlson_Comorbidity_Index_1d +
                        Age:CGS_Score + Charlson_Comorbidity_Index_1d:SOFA_1d + Charlson_Comorbidity_Index_1d:CGS_Score, 
                      data = data_center, 
...
summary(model_dagcentr)
coef(model_dagcentr)
model_dagcentr %>% tidy(exponentiate = TRUE, conf.int = TRUE)
rename_labels <- c(
  "Age" = "Age",
  "SOFA_1d" = "SOFA Score (1st Day)",
  "Charlson_Comorbidity_Index_1d" = "Charlson Comorbidity Index (1st Day)",
  ...
  "oneday_log" = "Extracellular DNA",
  "8ohdg_1d_log" = "DNA deoxyguanosine",
  "Age:SOFA_1d" = "Age × SOFA Score (1st Day)",
  "Age:Charlson_Comorbidity_Index_1d" = "Age × Charlson Comorbidity Index (1st Day)",
  "Age:CGS_Score" = "Age × CGS Score",
  "Charlson_Comorbidity_Index_1d:SOFA_1d" = "Charlson Comorbidity Index (1st Day) × SOFA Score (1st Day)",
  "Charlson_Comorbidity_Index_1d:CGS_Score" = "Charlson Comorbidity Index (1st Day) × CGS Score"
)

# Generate the plot with new model
p2 <- plot_model(model_dagcentr, 
                sort.est = TRUE, 
                show.values = TRUE, 
                show.p = TRUE, 
                ci.lvl = 0.95, 
                title = "Clinical Scales, Extracellular DNA, Interaction Effects, and Outcome",
                axis.labels = rev(rename_labels))  

p2

data_clean_center <- na.omit(data_center[, c("Outcome", "Age", "SOFA_1d", 
                                             "Charlson_Comorbidity_Index_1d", 
                                             "CGS_Score", "oneday_log", "8ohdg_1d_log")])
model_reduced_combo <- glm(Outcome ~ Age + SOFA_1d + Charlson_Comorbidity_Index_1d + 
                             CGS_Score + oneday_log + `8ohdg_1d_log`, 
                           data = data_clean_center, 
                           family = binomial)

model_full_combo <- glm(Outcome ~ Age + SOFA_1d + Charlson_Comorbidity_Index_1d + 
                          CGS_Score + oneday_log + `8ohdg_1d_log` +
                          Age:SOFA_1d + Age:Charlson_Comorbidity_Index_1d +
                          Charlson_Comorbidity_Index_1d:CGS_Score + Charlson_Comorbidity_Index_1d:SOFA_1d + Age:CGS_Score,
                        data = data_clean_center, 
                        family = binomial)
...
Boruta algorithm, KNN imputation and logistic regression for Outcome
...
data <- data %>% 
  rename(
    CIRS_G_Sum_1d = `CIRS-G_сумма_1d`, 
    CIRS_G_Severity_Index_1d = `CIRS-G_индекс_тяжести_1d`, 
    Pneumonia = Пневмония, 
    Sex = Пол, 
    FSCCRR_bed_days = `Койко-дни_ФНКЦРР`,
  )
data<- data %>%
  rename(
    Sepsis_Septicshock = `Сепсис/септический_шок`,
    Uroseptic_infection = `Уросептическая_инфекция`,
    Infection = `Инфекция`
  )

data_for_regres2 <- data[, c("Sex", "Age", "Pneumonia", 
                             "Infection", "CGS_Score", "Charlson_Comorbidity_Index_1d", 
                             "CIRS_G_Sum_1d", "CIRS_G_Severity_Index_1d", "FOUR_1d", "SOFA_1d", 
                             "WBC_1d", "NEU_#_1d", "LYM_#_1d", "PLT_1d", "Lactate_1d", "Creatinine_1d", 
                             "ALT_1d", "AST_1d", "Albumin_1d", "Alkaline_Phosphatase_1d", "Uric_Acid_1d", 
                             "Urea_1d", "GGT_1d", "Potassium_1d", "Sodium_1d", "Chloride_1d", 
                             "APTT_1d",  "Fibrinogen_1d",  "Uroseptic_infection", 
                             "Sepsis_Septicshock",  "Ventilation_d", "FSCCRR_bed_days", 
                             "Outcome", "oneday_log", "8ohdg_1d_log")]
data_for_regres2$Sex <- toupper(data_for_regres2$Sex)
data_for_regres2$Sex <- ifelse(data_for_regres2$Sex == "М", "M", 
                               ifelse(data_for_regres2$Sex == "Ж", "F", data_for_regres2$Sex))

data_for_regres2 %>%
  mutate(across(`Sex`, function(x) x %>% factor(levels = c("M", "F"))))

data_for_regres2 %>%
  mutate(across(`Infection`, function(x) x %>% factor(levels = c("0", "1"))))
data_for_regres2 %>%
  mutate(across(`Pneumonia`, function(x) x %>% factor(levels = c("0", "1"))))

data_for_regres2 <- data_for_regres2 %>%
  mutate(`Sepsis_Septicshock` = ifelse(`Sepsis_Septicshock` %in% c("0", "1"), 
                                       `Sepsis_Septicshock`, 
                                       NA))
data_for_regres2 %>%
  mutate(across(`Sepsis_Septicshock`, function(x) x %>% factor(levels = c("0", "1"))))
data_for_regres2 %>%
  mutate(across(`Uroseptic_infection`, function(x) x %>% factor(levels = c("0", "1"))))
data_for_regres2 %>%
  mutate(across(`Outcome`, function(x) x %>% factor(levels = c("0", "1"))))

data_for_regres2 <- data_for_regres2 %>%
  mutate(across(c(`Sex`, `Infection`, `Pneumonia`, `Sepsis_Septicshock`, 
                  `Uroseptic_infection`, `Outcome`), as.factor))


numeric_columns <- c("Age", "CGS_Score", "Charlson_Comorbidity_Index_1d",
                     "CIRS_G_Sum_1d", 
                     "CIRS_G_Severity_Index_1d", "FOUR_1d", "SOFA_1d", 
                     "WBC_1d", "NEU_#_1d", "LYM_#_1d", "PLT_1d", 
                     "Lactate_1d", "Creatinine_1d", "ALT_1d", "AST_1d", 
                     "Albumin_1d", "Alkaline_Phosphatase_1d", "Uric_Acid_1d", 
                     "Urea_1d", "GGT_1d", "Potassium_1d", "Sodium_1d", 
                     "Chloride_1d", "APTT_1d","Fibrinogen_1d", 
                     "Ventilation_d", "FSCCRR_bed_days", 
                     "oneday_log", "8ohdg_1d_log")


data_for_regres2[numeric_columns] <- lapply(data_for_regres2[numeric_columns], as.numeric)

skimr::skim(data_for_regres2)


missing_proportion <- colSums(is.na(data_for_regres2)) / nrow(data_for_regres2)


cols_to_remove <- names(missing_proportion[missing_proportion > 0.5])


data_for_regres3 <- data_for_regres2[, !(names(data_for_regres2) %in% cols_to_remove)]

recipe_obj <- recipe(Outcome ~ ., data = data_for_regres3) %>%
  step_impute_knn(all_predictors(), neighbors = 5) %>%
  prep(training = data_for_regres3)


data_for_regres_imputed <- bake(recipe_obj, new_data = data_for_regres3)
str(data_for_regres3)

data_for_regres3 %>% glimpse()

data_for_regres_imputed$Infection <- as.character(data_for_regres_imputed$Infection)


data_for_regres_imputed$Infection[!(data_for_regres_imputed$Infection %in% c("1", "0"))] <- "1"

table(data_for_regres_imputed$Infection)

data_clean <- data_for_regres_imputed[!is.na(data_for_regres_imputed$Outcome), ]
num_rows_left <- nrow(data_clean)
print(num_rows_left)
boruta_result <- Boruta(Outcome ~ ., data = data_clean, doTrace = 2)
print(boruta_result)
final_features <- getSelectedAttributes(boruta_result, withTentative = TRUE)
print(final_features)
data_selected <- data_clean[, c(final_features, "Outcome")]
model_glm_boruta <- glm(Outcome ~ ., data = data_selected, family = binomial)
summary(model_glm_boruta)

tidy_stepwise <- tidy(model_glm_boruta, exponentiate = TRUE, conf.int = TRUE)
glance_results <- glance(model_glm_boruta)
...
rename_labels_new <- c(
  ...
  "Charlson_Comorbidity_Index_1d" = "Charlson Comorbidity Index (1st Day)",
  "AST_1d" = "AST (1st Day)",
  "Sodium_1d" = "Sodium (1st Day)",
  "Uroseptic_infection" = "Uroseptic Infection",
  "Sepsis_Septicshock" = "Sepsis & Septic Shock",
  "Ventilation_d" = "Ventilation (Days)",
  "FSCCRR_bed_days" = "FSCCRR Bed Days",
  "oneday_log" = "Extracellular DNA",
  "SexM" = "Male Sex",  
  "Infection1" = "Infection present",  
  "FOUR_1d" = "FOUR Score (1st Day)",  
  "Sodium_1d" = "Sodium (1st Day)",  
  "Uroseptic_infection1" = "Uroseptic Infection",  
  "Sepsis_Septicshock1" = "Sepsis & Septic Shock",  
  "Ventilation_d" = "Ventilation (Days)",  
  "FSCCRR_bed_days" = "FSCCRR Bed Days",
  "8ohdg_1d_log" = "DNa deoxyguanosine",
  "WBC_1d" = "WBC (1st Day)",  
  "NEU_#_1d" = "Neutrophils (1st Day)",  
  "LYM_#_1d" = "Lymphocytes (1st Day)",  
  "PLT_1d" = "Platelets (1st Day)",  
  "Lactate_1d" = "Lactate (1st Day)",  
  "Creatinine_1d" = "Creatinine (1st Day)",  
  "ALT_1d" = "ALT (1st Day)", 
  "Albumin_1d" = "Albumin (1st Day)",  
  "Alkaline_Phosphatase_1d" = "Alkaline Phosphatase (1st Day)",  
  "Uric_Acid_1d" = "Uric Acid (1st Day)", 
  "Urea_1d" = "Urea (1st Day)", 
  "GGT_1d" = "GGT (1st Day)",  
  "Potassium_1d" = "Potassium (1st Day)",
  "Pneumonia1" = "Pneumonia",
  "Fibrinogen_1d" = "Fibrinogen (1st Day)",
  "Chloride_1d" = "Chloride (1st Day)",
  "SOFA_1d" = "SOFA (1st Day)",
  "CIRS_G_Sum_1d" ="CIRSG Sum (1st Day)", 
  "CIRS_G_Severity_Index_1d" = "CIRSG Severity (1st Day)"
  
  # FSCCRR_bed_days
)

p_new <- plot_model(model_glm_boruta, 
                    sort.est = TRUE, 
                    show.values = TRUE, 
                    show.p = TRUE, 
                    ci.lvl = 0.95, 
                    title = "Logistic regression results",
                    axis.labels = rev(rename_labels_new),
                    value.offset = 0.5)
```

Boruta algorithm, KNN imputation and logistic regression for Ventilation need
...
data1 <- data1 %>% rename(Ventilation_Need = Потребность_ИВЛ)



data_for_regres4 <- data1[, c("Sex", "Age", "Pneumonia", 
                              "Infection", "CGS_Score", "Charlson_Comorbidity_Index_1d", 
                              "CIRS_G_Sum_1d", "CIRS_G_Severity_Index_1d", "FOUR_1d", "SOFA_1d", 
                              "WBC_1d", "NEU_#_1d", "LYM_#_1d", "PLT_1d", "Lactate_1d", "Creatinine_1d", 
                              "ALT_1d", "AST_1d", "Albumin_1d", "Alkaline_Phosphatase_1d", "Uric_Acid_1d", 
                              ...
                              "APTT_1d",  "Fibrinogen_1d",  "Uroseptic_infection", 
                              "FSCCRR_bed_days", 
                              "oneday_log", "8ohdg_1d_log", "Ventilation_Need")]
data_for_regres4$Sex <- toupper(data_for_regres4$Sex)
data_for_regres4$Sex <- ifelse(data_for_regres4$Sex == "М", "M", 
                               ifelse(data_for_regres4$Sex == "Ж", "F", data_for_regres4$Sex))

data_for_regres4 %>%
  mutate(across(`Ventilation_Need`, function(x) x %>% factor(levels = c("0", "1"))))
data_for_regres4 %>%
  mutate(across(`Sex`, function(x) x %>% factor(levels = c("M", "F"))))
...
data_for_regres4 <- data_for_regres4 %>%
  mutate(across(c(`Sex`, `Infection`, `Pneumonia`,  
                  `Uroseptic_infection`,`Ventilation_Need` ), as.factor))
numeric_columns <- c("Age", "CGS_Score", "Charlson_Comorbidity_Index_1d",
                     "CIRS_G_Sum_1d", 
                     "CIRS_G_Severity_Index_1d", "FOUR_1d", "SOFA_1d", 
                     "WBC_1d", "NEU_#_1d", "LYM_#_1d", "PLT_1d", 
...
data_for_regres4[numeric_columns] <- lapply(data_for_regres4[numeric_columns], as.numeric)

skimr::skim(data_for_regres4)



missing_proportion <- colSums(is.na(data_for_regres4)) / nrow(data_for_regres4)


cols_to_remove <- names(missing_proportion[missing_proportion > 0.5])


data_for_regres5 <- data_for_regres4[, !(names(data_for_regres4) %in% cols_to_remove)]


#data_imputed <- kNN(data_for_regres3, k = 5)
library(recipes)
recipe_obj <- recipe(Ventilation_Need ~ ., data = data_for_regres5) %>%
  step_impute_knn(all_predictors(), neighbors = 5) %>%
  prep(training = data_for_regres5)


data_for_regres_imputed2 <- bake(recipe_obj, new_data = data_for_regres5)
str(data_for_regres5)

data_for_regres5 %>% glimpse()
data_for_regres_imputed2$Infection <- ifelse(data_for_regres_imputed2$Infection %in% 
                                               c("интерстициальный цистит", 
                                                 "полисегментарная пневмония справа, ст. разрешения", 
                                                 "полисегментарная правосторонняя пневмония"), 
                                             1, 
                                             data_for_regres_imputed2$Infection)



data_clean <- data_for_regres_imputed2[!is.na(data_for_regres_imputed2$Ventilation_Need), ]

boruta_result <- Boruta(Ventilation_Need ~ ., data = data_clean, doTrace = 2)
print(boruta_result)
final_features <- getSelectedAttributes(boruta_result, withTentative = FALSE)
print(final_features)
data_selected <- data_clean[, c(final_features, "Ventilation_Need")]
model_glm_boruta <- glm(Ventilation_Need ~ ., data = data_selected, family = binomial)
summary(model_glm_boruta)
...
rename_labels_new <- c(
  ...
  "oneday_log" = "Extracellular DNA",
  "SexM" = "Male Sex",  # SexM
  "Infection1" = "Infection present",  # Infection1
  "FOUR_1d" = "FOUR Score (1st Day)",  # FOUR_1d
  "Sodium_1d" = "Sodium (1st Day)",  
  "Uroseptic_infection1" = "Uroseptic Infection",  # Uroseptic_infection1
  "Sepsis_Septicshock1" = "Sepsis & Septic Shock",  # Sepsis_Septicshock1
  "Ventilation_d" = "Ventilation (Days)",  # Ventilation_d
  "FSCCRR_bed_days" = "FSCCRR Bed Days",
  "8ohdg_1d_log" = "DNA deoxyguanosine",
  "WBC_1d" = "WBC (1st Day)",  
  "NEU_#_1d" = "Neutrophils (1st Day)",  
...
p_new <- plot_model(model_glm_boruta, 
                    ...
                    title = "Logistic regression results",
                    axis.labels = rev(rename_labels_new),
                    value.offset = 0.5)  # Ensure correct order
p_new
check_model(model_glm_boruta, type = "discrete_dots")
```
...
## Устин Competing risk regression
...
data <- data %>% 
  mutate(
    Пол = as.factor(Пол),
    HTR1A_rs6295 = as.factor(HTR1A_rs6295),
    AQP4_rs1058427 = as.factor(AQP4_rs1058427),
    BDNF_rs6265 = as.factor(BDNF_rs6265),
    OLR1_rs11053646 = as.factor(OLR1_rs11053646),
    NRF2_rs6726395 = as.factor(NRF2_rs6726395),
    AGTR1_rs275651 = as.factor(AGTR1_rs275651),
    AQP5_rs3759129 = as.factor(AQP5_rs3759129),
    TLR9_rs352162 = as.factor(TLR9_rs352162),
    AQP_rs3736309 = as.factor(AQP_rs3736309),
    `анти-HCV` = as.factor(`анти-HCV`),
    Hbs_AG = as.factor(Hbs_AG),
    ЛСМА = as.factor(ЛСМА %>% na_if("?")),
    Основной_диагноз_код_МКБ10 = as.factor(Основной_диагноз_код_МКБ10),
    Пневмония = as.factor(Пневмония),
    Инфекция = as.factor(Инфекция),
    CGS_балл = as.factor(CGS_балл),
    Пневмония_возникшая_ФНКЦРР = as.factor(Пневмония_возникшая_ФНКЦРР),
    Исход = as.factor(Исход),
    Уросептическая_инфекция = as.factor(Уросептическая_инфекция),
    `Сепсис/септический_шок` = as.factor(`Сепсис/септический_шок`),
    `Потребность_ИВЛ` = as.factor(`Потребность_ИВЛ`),
    `Ухудшение_пневмонии_сутки` = as.factor(`Ухудшение_пневмонии_сутки`),
    TLR9_rs187084 = as.factor(TLR9_rs187084),
    t_при_поступлении = as.double(t_при_поступлении),
    across(starts_with("Индекс_коморбидности_Чарлсона"), as.numeric),
    across(starts_with("CIRS-G"), as.double),
    across(starts_with("FOUR"), as.double),
    across(starts_with("SOFA"), as.double),
    across(starts_with("WBC"), as.double),
    across(starts_with("NEU"), as.double),
    across(starts_with("LYM"), as.double),
    across(starts_with("PLT"), as.double),
    across(starts_with("Лактат"), as.double),
    across(starts_with("Креатинин"), as.double),
    across(starts_with("АЛТ"), as.double),
    across(starts_with("АСТ"), as.double),
    across(starts_with("Альбумин"), as.double),
    across(starts_with("Щелочная_фосфотаза"), as.double),
    across(starts_with("Мочевая_кислота"), as.double),
    across(starts_with("Мочевина"), as.double),
    across(starts_with("ГГТ"), as.double),
    across(starts_with("Калий"), as.double),
    across(starts_with("Натрий"), as.double),
    across(starts_with("Хлорид"), as.double),
    across(starts_with("АЧТВ"), as.double),
    across(starts_with("МНО"), as.double),
    across(starts_with("Фибриноген"), as.double),
    across(starts_with("ПВ"), as.double),
    `Койко-дни_ФНКЦРР` = as.numeric(`Койко-дни_ФНКЦРР`),
    Дни_ОРИТ_ФНКЦРР = as.numeric(Дни_ОРИТ_ФНКЦРР),
    ИВЛ_д = as.numeric(ИВЛ_д)
    )

cldata <- data %>% select(-c(Номер, Часть, Номер_референс, ID, starts_with("Дата"), внкДНК_1d, внкДНК_5d, `8ohdg_1d`, `8ohdg_5d`, `8ohdg на миллион оснований 1d лог`, `8ohdg 5d лог`, Диагноз, `Койко-дни_ФНКЦРР`, Исход ))

# cldata <- cldata %>% mutate(across(where(is.numeric), ~ ifelse(is.na(.), median(., na.rm = TRUE), .)),
#                             across(where(is.factor), ~ ifelse(is.na(.), mode(.), .)),
...
Оценим количество потенциальных кластеров

```{r}
cldata_new <- cldata %>% select(-ЛСМА, -Основной_диагноз_код_МКБ10, -ends_with("_5d"), -ends_with("_21d"), -ends_with("_out"), -ends_with("_3d"), -ends_with("_5d_35"), -ends_with("_5d_36"), -ends_with("_5d_37"), -ends_with("_5d_38"), -ends_with("_5d_39"), -ends_with("_5d_40"), -Ухудшение_пневмонии_сутки, -fiveday_log, -`8ohdg 5d log`) %>%
  mutate(t_при_поступлении = case_when(
    nchar(t_при_поступлении) == 3 ~ t_при_поступлении / 10,
    TRUE ~ t_при_поступлении
  ))

slh <- c()
slh = c(slh, NA)
ddist <- daisy(cldata_new, metric="gower")
for(i in 2:100){
  pam_clusters = pam(ddist,
                     diss = TRUE,
                     k = i)
  slh = c(slh ,pam_clusters$silinfo$avg.width)
}
plot(1:100, slh,
     xlab = "Clusters",
...
```{r}
pvdf <- tibble(nclusters = numeric(), Исход = numeric(), `Койко-дни` = numeric())
for (i in 2:70) {
   res <- pam(ddist, k=i, diss=T)
   pv <- chisq.test(table(res$clustering, data$Исход))$p.value
   pv2 <- tidy(aov(data$`Койко-дни_ФНКЦРР`~res$clustering))$p.value[1]
   pvdf <- pvdf %>% add_row(nclusters=i, Исход=pv, `Койко-дни`=pv2)
}

pvdf %>% rename(Outcome="Исход", Days_in_clinic="Койко-дни")
```
```{r}
pam_res <- pam(ddist, k=4, diss=T)
```


```{r}
ggplot(data %>% mutate(Cluster = pam_res$clustering), aes(x = Исход, fill = factor(Cluster))) +
  geom_bar(position = "dodge") +
  geom_text(aes(label=Cluster, x =Исход), stat="count", position=position_dodge(0.9)) +
  labs(x = "Исход", fill = "Кластер") %>% 
  scale_x_discrete(labels = c("0" = "Alive", "1" = "Death")) %>% 
  labs(x="Outcome", y = "Count", fill = "Cluster")
  
```
```{r}
data %>% mutate(
  Cluster = pam_res$clustering,
  Дни = cut(`Койко-дни_ФНКЦРР`,
                       breaks = c(-Inf, 25, 50, 75, 100, 125, 150, 175, 200, Inf),  # Интервалы
                       labels = c("< 25", "25-50", "50-75", "75-100", "100-125", "125-150", "150-175", "175-200", "> 200"),
                       right = TRUE)
  ) %>% 
  ggplot(aes(x = Дни, fill = factor(Cluster))) +
    geom_bar(position = "dodge") +
    geom_text(aes(label=Cluster, x =Дни), stat="count", position=position_dodge(0.9)) +
    labs(x = "Дни в ФНКЦ РР", fill = "Кластер") +
    labs(x="Days in clinic", y = "Count", fill = "Cluster")
```

```{r}
nums <- cldata_new %>% mutate(Cluster = pam_res$clustering) %>% 
  select(where(is.numeric)) %>% 
  rename_with(~ str_replace_all(., "CIRS-G", "CIRS_G")) %>% 
  rename_with(~ str_replace_all(., "_#", "")) %>% 
  rename_with(~ str_replace_all(., "_%", "_perc")) %>% 
  rename_with(~ str_replace_all(., "8ohdg 1d log", "OHDG8_log_1d"))

for (var in colnames(nums)) {
    formula <- as.formula(paste(var, "~", "Cluster"))
    anova_result <- aov(formula, data = nums)
    results <- summary(anova_result)[[1]]$`Pr(>F)`[1]
    print(paste(results, var))
}
```
```{r}
vis_var <- c( "Возраст", "Индекс_коморбидности_Чарлсона_1d", "CIRS_G_сумма_1d", "CIRS_G_систем_с_паталогиями_1d", "FOUR_1d", "SOFA_1d", "NEU_perc_1d", "LYM_perc_1d", "ИВЛ_д", "ИВЛ_д")
...
for (var in vis_var) {
  print(data %>% 
    ...
    rename_with(~ str_replace_all(., "_#", "")) %>% 
    rename_with(~ str_replace_all(., "_%", "_perc")) %>% 
    rename_with(~ str_replace_all(., "8ohdg 1d log", "OHDG8_log_1d")) %>% mutate(
    Cluster = as.factor(pam_res$clustering)
    ) %>% 
    ggplot()+
    geom_boxplot(aes_string(y = var, x="Cluster")))
}
