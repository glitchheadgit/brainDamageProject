...
hospital_polymorf_e <- data_e %>%  mutate (`HTR1A_rs6295` = ifelse (`HTR1A_rs6295` == 'С/С', 'C/C', `HTR1A_rs6295`),
  ...
  dplyr::select(ID, HTR1A_rs6295, AQP4_rs1058427, BDNF_rs6265, OLR1_rs11053646, NRF2_rs6726395, AGTR1_rs275651, AQP5_rs3759129, TLR9_rs352162, TLR9_rs187084, AQP5_rs3736309, `Ventilation`, `Outcome`, `oneday_log`, `8ohdg 1d log`, `NLR_1d` ) %>% 
...
```{r}
data_no_na_dates <- data_e %>% filter(!is.na(Discharge_date_FSCCRR))

forcrmp <- data_no_na_dates %>%
  mutate(
    time = `FSCCRR_bed-days` %>% as.numeric(),
    status = case_when(
      Outcome == "0" ~ "Выписка",
      Outcome == "1" ~ "Смерть",
      Outcome == "2" ~ "Цензура"
  ...
  select(time, status, Uroseptic_infection, diseaseGroup, Outcome)
```
...
data <- data_e %>% 
  mutate(
    ...
    AQP5_rs3736309 = as.factor(AQP5_rs3736309),
    `anti-HCV` = as.factor(`anti-HCV`),
    Hbs_AG = as.factor(Hbs_AG),
    LMCA = as.factor(LMCA %>% na_if("?")),
    Main_diagnosis_code = as.factor(Main_diagnosis_code),
    Pneumonia = as.factor(Pneumonia),
    Infection = as.factor(Infection),
    CGS = as.factor(CGS),
    Pneumonia_at_FSCCRR = as.factor(Pneumonia_at_FSCCRR),
    Outcome = as.factor(Outcome),
    Uroseptic_infection = as.factor(Uroseptic_infection),
    `Sepsis/septic_shock` = as.factor(`Sepsis/septic_shock`),
    `Ventilation` = as.factor(`Ventilation`),
    TLR9_rs187084 = as.factor(TLR9_rs187084),
    T_admission = as.double(T_admission),
    across(starts_with("CCI"), as.numeric),
    ...
    across(starts_with("Lactate"), as.double),
    across(starts_with("Creatinine"), as.double),
    across(starts_with("ALT"), as.double),
    across(starts_with("AST"), as.double),
    across(starts_with("Albumin"), as.double),
    across(starts_with("Alcaline_phosphatase"), as.double),
    across(starts_with("Uric_acid"), as.double),
    across(starts_with("Urea"), as.double),
    across(starts_with("GGT"), as.double),
    across(starts_with("Potassium"), as.double),
    across(starts_with("Sodium"), as.double),
    across(starts_with("Chloride"), as.double),
    across(starts_with("APTT"), as.double),
    across(starts_with("INR"), as.double),
    across(starts_with("Fibrinogen"), as.double),
    across(starts_with("PT"), as.double),
    `FSCCRR_bed-days` = as.numeric(`FSCCRR_bed-days`),
    ICU_FSCCRR_d = as.numeric(ICU_FSCCRR_d),
    Ventilation_d = as.numeric(Ventilation_d)
  )

cldata <- data %>% select(-c(Num, Part, Num_ref, ID, starts_with("Date"), `8ohdg_1d`, `8ohdg_5d`, `8ohdg 1d log`, `8ohdg 5d log`, Diagnosis, `FSCCRR_bed-days`, Outcome, Discharge_date_FSCCRR, Transfer_date_FSCCRR, Before_FSCCRR_d))
...
Assess number of potential clusters

```{r}
cldata_new <- cldata %>% select(-LMCA, -Main_diagnosis_code, -ends_with("_5d"), -ends_with("_21d"), -ends_with("_out"), -ends_with("_3d"), -ends_with("_5d_35"), -ends_with("_5d_36"), -ends_with("_5d_37"), -ends_with("_5d_38"), -ends_with("_5d_39"), -ends_with("_5d_40"), -Pneumonia_deterioration_d, -fiveday_log) %>%
  mutate(T_admission = case_when(
    nchar(T_admission) == 3 ~ as.double(T_admission) / 10,
    nchar(T_admission) == 2 ~ as.double(T_admission),
    TRUE ~ NA
  ))
...
```{r}
pvdf <- tibble(nclusters = numeric(), Outcome = numeric(), `FSCCRR_bed-days` = numeric())
for (i in 2:70) {
   res <- pam(ddist, k=i, diss=T)
   pv <- chisq.test(table(res$clustering, data$Outcome))$p.value
   pv2 <- tidy(aov(data$`FSCCRR_bed-days`~res$clustering))$p.value[1]
   pvdf <- pvdf %>% add_row(nclusters=i, Outcome=pv, `FSCCRR_bed-days`=pv2)
}

pvdf %>% rename(Outcome="Outcome", Days_in_clinic="FSCCRR_bed-days")
```
```{r}
...
```{r}
ggplot(data %>% mutate(Cluster = pam_res$clustering), aes(x = Outcome, fill = factor(Cluster))) +
  geom_bar(position = "dodge") +
  geom_text(aes(label=Cluster, x =Outcome), stat="count", position=position_dodge(0.9)) +
  labs(x = "Outcome", fill = "Cluster") %>% 
...
data %>% mutate(
  ...
  ggplot(aes(x = Days, fill = factor(Cluster))) +
    geom_bar(position = "dodge") +
    geom_text(aes(label=Cluster, x =Days), stat="count", position=position_dodge(0.9)) +
    labs(x = "Days in FSCCRR", fill = "Cluster") +
...
```
```{r}
vis_var <- c("Age", "CCI_1d", "CIRS_G_sum_1d", "CIRS_G_pathology_1d", "FOUR_1d", "SOFA_1d", "NEU_perc_1d", "LYM_perc_1d", "Ventilation_d", "Ventilation_d")
...
Prepare dataset for for PCA analysis and univariate regression, including 0-day status, demographic data, disease group and NLR. Also create a new categorial variable for bed-days at FSCCRR based on quantiles , called **BD_group**
...
discharged <- hosp_poly_group_nlr %>% filter(Outcome == "0") %>%  select (!c(Outcome, `Ventilation_d`)) %>%  filter(!is.na(Ventilation))
...
full_nums <- discharged %>% select (!c(LMCA, `8ohdg_1d`,`Lactate_1d`,
                                                `Alcaline_phosphatase_1d`, `Uric_acid_1d`, `GGT_1d`, 
                                                `Chloride_1d`, `Pneumonia_deterioration_d`, `8ohdg 1d log`,`FSCCRR_bed-days`)) %>% 
                                    select (BD_group, Ventilation, diseaseGroup, where(is.numeric)) %>% drop_na()
# got a tibble:302 × 29
```
...
small_pca <- discharged  %>% select (Ventilation, BD_group, diseaseGroup, Age, CGS, CCI_1d,  `CIRS-G_sum_1d`, 
...
components2$PC3 <- -components2$PC3
...
hospital.pca <- prcomp(small_pca[,-(1:3)], 
                scale = T )
summary (hospital.pca)
components3 <- hospital.pca[["x"]]
     
components3 <- data.frame(components2[,1:4])
components3$PC2 <- -components3$PC2
components3$PC3 <- -components3$PC3
components3 = cbind(components3, small_pca$BD_group)
colnames(components3)[5] = 'Bed-days'


tit = 'Total Explained Variance for PC1-PC4 = 87'
...
# mutate(across(c("Age", "SOFA_1d", "CCI_1d", 
#                "CGS", "oneday_log", "8ohdg 1d log"), as.numeric))

data$Outcome <- as.numeric(data$Outcome)
#drop_na(data)
data <- data[data$Outcome %in% c(0, 1), ]
table(data$Outcome)
str(data$Age)
data$SOFA_1d <- as.numeric(data$SOFA_1d)
data$CCI_1d <- as.numeric(data$CCI_1d)
data$CGS <- as.numeric(data$CGS)
data$oneday_log <- as.numeric(data$oneday_log)
data$`8ohdg 1d log` <- as.numeric(data$`8ohdg 1d log`)
data$Age <- as.numeric(data$Age)
model <- glm(Outcome ~ Age + SOFA_1d + CCI_1d + 
               CGS + oneday_log + `8ohdg 1d log`, 
             data = data, 
             family = binomial)
######LR ratio
data_clean <- na.omit(data[, c("Outcome", "Age", "SOFA_1d", 
                               "CCI_1d", 
                               "CGS", "oneday_log", "8ohdg 1d log")])

model_full <- glm(Outcome ~ Age + SOFA_1d + CCI_1d + 
                  CGS + oneday_log + `8ohdg 1d log`, 
                  data = data_clean, 
                  family = binomial)
...
model_dag <- glm(Outcome ~ Age + SOFA_1d + CCI_1d + 
                   CGS + oneday_log + `8ohdg 1d log` +
                   Age:SOFA_1d + Age:CCI_1d +
                   Age:CGS + oneday_log:SOFA_1d + oneday_log:CCI_1d + 
                   `8ohdg 1d log`:SOFA_1d + `8ohdg 1d log`:CCI_1d, 
                 data = data, 
                 family = binomial)
model_dag
...
data_center$CCI_1d <- scale(data$CCI_1d, center = TRUE, scale = FALSE)
data_center$CGS <- scale(data$CGS, center = TRUE, scale = FALSE)
...
model_dagcentr <- glm(Outcome ~ Age + SOFA_1d + CCI_1d + 
                        CGS + oneday_log + `8ohdg 1d log` +
                        Age:SOFA_1d + Age:CCI_1d +
                        Age:CGS + oneday_log:SOFA_1d + oneday_log:CCI_1d + 
                        `8ohdg 1d log`:SOFA_1d + `8ohdg 1d log`:CCI_1d, 
...
data_clean_center <- na.omit(data_center[, c("Outcome", "Age", "SOFA_1d", 
                               "CCI_1d", 
                               "CGS", "oneday_log", "8ohdg 1d log")])
model_reduced_combo <- glm(Outcome ~ Age + SOFA_1d + CCI_1d + 
                      CGS + oneday_log + `8ohdg 1d log`, 
                    data = data_clean_center, 
                    family = binomial)

model_full_combo <- glm(Outcome ~ Age + SOFA_1d + CCI_1d + 
                    CGS + oneday_log + `8ohdg 1d log` +
                    Age:SOFA_1d + Age:CCI_1d +
                    Age:CGS + oneday_log:SOFA_1d + oneday_log:CCI_1d + 
                    `8ohdg 1d log`:SOFA_1d + `8ohdg 1d log`:CCI_1d, 
...
```
#### Lena univariate regression for all variables:
```{r}
# prepare such a dataset better somewhere above (all variables + include grouping, processed polymorphisms and NLR)
data_for_regres <- data %>% select (ID, Sex, Age, `anti-HCV`, `Hbs_AG`,  `LMCA`, Pneumonia, Infection, diseaseGroup, CGS, CCI_1d, `CIRS-G_sum_1d`, `CIRS-G_pathology_5d_36`, `CIRS-G_severity_1d`, FOUR_1d, SOFA_1d, exDNA_1d, `8ohdg_1d`, WBC_1d,  `NEU_#_1d`, `NEU_%_1d`, `LYM_#_1d`, `LYM_%_1d`,  `PLT_1d`, `Lactate_1d`, Creatinine_1d, ALT_1d, AST_1d,  Albumin_1d, Alcaline_phosphatase_1d , Uric_acid_1d, Urea_1d, GGT_1d, Potassium_1d, Sodium_1d, Chloride_1d,  APTT_1d, INR_1d, Fibrinogen_1d, PT_1d, Uroseptic_infection, `Sepsis/septic_shock`, `Ventilation`, `Pneumonia_worsening`, `Ventilation_d`, `ICU_FSCCRR_d`, `FSCCRR_bed-days`, `Outcome`, `Pneumonia_at_FSCCRR`, `oneday_log`, `8ohdg 1d log` ) %>% mutate(across(c(`CIRS-G_sum_1d`, `CIRS-G_pathology_5d_36`,`CIRS-G_severity_1d`,`FOUR_1d`, `WBC_1d`, `NEU_#_1d`, `NEU_%_1d`, `LYM_#_1d`, `LYM_%_1d`, `PLT_1d`, `Lactate_1d`, `Creatinine_1d` , `ALT_1d`, `AST_1d`, `Albumin_1d`, `Alcaline_phosphatase_1d`, `Uric_acid_1d`, `Urea_1d` , `GGT_1d`, `Potassium_1d`, `Sodium_1d`, `Chloride_1d`, `APTT_1d`, `INR_1d`,`Fibrinogen_1d`, PT_1d , `Pneumonia_worsening`, `Ventilation_d`, `ICU_FSCCRR_d`, `FSCCRR_bed-days`), as.numeric), across(c(Outcome,`Ventilation`, factor)) 
...
#Got a complete dataset, including new variables and edited source variables with the correct type
summary(hosp_poly_group_nlr)
colnames(hosp_poly_group_nlr)
```

Let's construct a univariate regression for outcome

```{r}
x = names(hosp_poly_group_nlr[,-48])
out <- unlist(lapply(48, function(n) combn(x, 1, FUN=function(row) paste0("Outcome ~ ", paste0("`",row, "`",collapse = "+")))))
out
...
#write.xlsx(signif_estim_univar, 'signif_results_univar_glm.xlsx')
```
### Regression analysis for association finding:

Test interaction hypotheses (for the whole sample and in strata)
○ TLR9 gene (receptor) and extracellular DNA polymorphism 
○ Polymorphism of the AQP4 gene and leucocyte NLR index

##### Relationship of TLR9 gene polymorphism (receptor) and extracellular DNA 
...
```{r}
disease_group_id <- data %>% select (ID, Age, SOFA_1d,  diseaseGroup, oneday_log, `8ohdg 1d log`, CGS, CCI_1d, `FSCCRR_bed-days`)

#  `Outcome` WITHOUT GROUPING
fit_tlr <-   glm (Outcome ~ TLR9_rs352162 :oneday_log + TLR9_rs187084 + `8ohdg 1d log`, data = hosp_poly_group_nlr, family = binomial)
summary(fit_tlr)
# Polymorphism/DNA: No significant association with outcome (discharge, death)

#  `Outcome` WITH GROUPING
fit_tlr_gr <-   glm (Outcome ~ TLR9_rs352162 + TLR9_rs187084 + oneday_log + `8ohdg 1d log` + diseaseGroup , data = hosp_poly_group_nlr, family = binomial)
summary(fit_tlr_gr)
# Polymorphism/DNA: Not associated... TBI worsens prognosis (p-value 0.05)

#  `Ventilation` WITH GROUPING
fit_tlr_1 <-   glm (Ventilation ~ TLR9_rs352162 + TLR9_rs187084 + oneday_log + `8ohdg 1d log` + diseaseGroup , data = hosp_poly_group_nlr, family = binomial)
summary(fit_tlr_1)
# Polymorphism/DNA: Not associated... Hypoxia increases the likelihood of ventilation need (p-value 0.05)

#  `FSCCRR_bed-days` WITH GROUPING
fit_tlr_2 <-   lm (FSCCRR_bed-days ~ TLR9_rs352162 + TLR9_rs187084 + oneday_log + `8ohdg 1d log` + diseaseGroup, data = hosp_poly_group_nlr)
summary(fit_tlr_2)
# Polymorphism/DNA: Not associated... TBI group has a positive association with bed-days, while Neoplasms group has a negative association.
```
...
#  `Outcome` WITH GROUPING
fit_nlr_gr <-   glm (Outcome ~  AQP4_rs1058427 + NLR_1d + diseaseGroup , data = hosp_poly_group_nlr, family = binomial)
summary(fit_nlr_gr)
# NLR_1 is associated with outcome (effect is minor?), stratum is not associated.

#  `Ventilation` WITH GROUPING
fit_nlr_gr1 <-   glm (Ventilation ~  AQP4_rs1058427 + NLR_1d + diseaseGroup , data = hosp_poly_group_nlr, family = binomial)
summary(fit_nlr_gr1)
## NLR_1 is associated with outcome (effect is minor?), neoplasms?

#  `FSCCRR_bed-days` WITH GROUPING
fit_nlr_2 <-   lm (FSCCRR_bed-days ~ AQP4_rs1058427 + NLR_1d + diseaseGroup, data = hosp_poly_group_nlr)
summary(fit_nlr_2)
# Polymorphism/NLR: Not associated... TBI consequences?
```
