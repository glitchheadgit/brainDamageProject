...
```{r}
ddist <- daisy(cldata, metric="gower")

# PAM кластеризация
pam_res <- pam(ddist, k = 4, diss=T)
pam_res$data <- cldata
pam_sil <- silhouette(pam_res$clustering, ddist)
mean(pam_sil[, 3])
...
data %>% mutate(
  ...
  Дни = cut(`Койко-дни_ФНКЦРР`,
                       ...
                       labels = c("меньше 25", "25-50", "50-75", "75-100", "100-125", "125-150", "150-175", "175-200", "больше 200"),
                       right = TRUE)
  ) %>% 
...
Выберем количество кластеров и визуализируем результаты

```{r}
hclusters <- cutree(hc, k=3)
fviz_cluster(list(data=cldata, cluster=hclusters))
```
